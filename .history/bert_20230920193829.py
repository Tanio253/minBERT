...
class BertAttention(nn.Module):
    ...
    def transform(self, x, f):
        ...
        return proj
    def attention(self, xq, xk, xv, masked_attention = None):
        # B: batch size
        # S: sequence length
        # E: embedding vector
        # H: number of heads
        ...
        B, _, L, _ = xq.shape
        attention_score = xq.mm(xk_transposed)/math.sqrt(self.head_size) # (B, H, S, S)
        ...
        attention_score = self.dropout(attention_score).mm(xv) # B, H, L, D
...
class BertLayer(nn.Module):
    ...
    def forward(self, x, masked_attention):
        f = self.MultiheadAttention(x, masked_attention = None)
