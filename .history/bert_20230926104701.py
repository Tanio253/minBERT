...
class BertAttention(nn.Module):
    ...
    def attention(self, xq, xk, xv, masked_attention):
        ...
        attention_score = attention_score.permute(0,2,1,3).reshape(B,S,-1) #B, S, E
...
class BertLayer(nn.Module):
    def __init__(self, config):
        ...
        self.attention_do = nn.Dropout(p = config.dropout_rate)
        self.ff_do = nn.Dropout(p = config.dropout_rate)
    def forward(self, x, masked_attention):
        attention_output = self.MultiheadAttention(x, masked_attention)
        attention_output = self.attention_do(self.attention_dense_layer(attention_output))
        attention_output = self.attention_layernorm(x+attention_output)
        ffoutput =self.ff_do(self.ff(attention_output))
        out = self.ff_layernorm(ffoutput+attention_output)
        return out
class BertModel(BertPreTrainedModel):
    ...
    def embed(self, input_ids):
        ...
        #position embedding
        pos_ids = self.pos_ids[:,:seq_len].to(input_ids.device)
        pos_embed = self.pos_embedding(pos_ids)
        #segment embedding
        segment_id = torch.zeros(input_ids.size(), dtype = torch.long, device = input_ids.device )
        segment_embed = self.segment_embedding(segment_id)
        out = token_embed+ pos_embed+ segment_embed
        out = self.embed_do(self.embed_norm(out))
        return out
