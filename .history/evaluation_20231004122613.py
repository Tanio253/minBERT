...
def MultiEvaluation(model, sst_dl, quora_dl, sts_dl):
    ...
    with torch.no_grad():
        for batch in tqdm(quora_dl, desc = f'Quora Evaluation'):
            input_ids1, attention_mask1, input_ids2, attention_mask2, labels, sent1, sent_id1, sent2, sent_id2 = batch
            ...
            sent_ids_quora1.extend(sent_id1)
            sents_quora2.extend(sent2)
            sent_ids_quora2.extend(sent_id2)
    ...
    with torch.no_grad():
        for batch in tqdm(sts_dl, desc = f'STS Evaluation'):
            ...
            sent_ids_sts1.extend(sent_id1)
            sents_sts2.extend(sent2)
            sent_ids_sts2.extend(sent_id2)
    ...
    return Dict(sst_accuracy= sst_accuracy, sst_sents = sents_sst, sst_sent_ids = sent_ids_sst, quora_accuracy = quora_accuracy,
                quora_sent1 = sents_quora1, quora_sent2 = sents_quora2, quora_sent_id1 = sent_ids_quora1, quora_sent_id2 = sent_ids_quora2,
                sts_pearson = sts_pearson, sts_sent1 = sents_sts1, sts_sent2 = sents_sts2, sts_sent_id1 = sent_ids_sts1, sts_sent_id2 = sent_ids_sts2
                )
...
def MultiEvaluationTest(model, sst_dl, quora_dl, sts_dl):
    ...
    with torch.no_grad():
        for batch in tqdm(quora_dl, desc = f'Quora Evaluation'):
            input_ids1, attention_mask1, input_ids2, attention_mask2, _, sent1, sent_id1, sent2, sent_id2 = batch
            logits = model.predict_paraphrase(input_ids1, attention_mask1, input_ids2, attention_mask2)
            y_pred = logits.cpu().numpy().flatten()
            y_pred_quora.extend(y_pred)
            sents_quora1.extend(sent1)
    ...
    return Dict(sst_pred = y_pred_sst, sst_sents = sents_sst, sst_sent_ids = sent_ids_sst, quora_pred = y_pred_quora,
                quora_sent1 = sents_quora1, quora_sent2 = sents_quora2, quora_sent_id1 = sent_ids_quora1, quora_sent_id2 = sent_ids_quora2,
                sts_pred = y_pred_sts, sts_sent1 = sents_sts1, sts_sent2 = sents_sts2, sts_sent_id1 = sent_ids_sts1, sts_sent_id2 = sent_ids_sts2
                )